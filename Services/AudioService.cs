...
namespace RecklessBoon.MacroDeck.Streamlabs_OBS_Plugin.Services
{
    ...
    {
        public async Task<AudioSource> GetSourceAsync(string sourceId) => await MakeCallAsync<AudioSource>(this.GetType().Name, new { sourceId });
        ...
        public async Task<AudioSource> GetSourcesForSceneAsync(string sourceId) => await MakeCallAsync<AudioSource>(this.GetType().Name, new { sourceId });
    }
}
