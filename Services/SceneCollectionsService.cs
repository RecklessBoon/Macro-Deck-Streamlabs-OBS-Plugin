...
namespace RecklessBoon.MacroDeck.Streamlabs_OBS_Plugin.Services
{
    public class SceneCollectionsService : BaseService
    {
        ...
        public async Task<SceneCollectionsManifestEntry> CreateAsync(SceneCollectionCreateOptions options) => await MakeCallAsync<SceneCollectionsManifestEntry>(this.GetType().Name, options);
        public async Task DeleteAsync(string id) => await MakeCallAsync(this.GetType().Name, id);
        ...
        public async Task LoadAsync(string id) => await MakeCallAsync(this.GetType().Name, id);
        public async Task RenameAsync(string newName, string id) => await MakeCallAsync(this.GetType().Name, Args(newName, id));
    }
}
