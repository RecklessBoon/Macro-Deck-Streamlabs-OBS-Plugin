...
| ---------------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| slobs_connected                    | Connection status (as true/false)<br><sub><i>True if you are connected to the Streamlabs OBS Desktop application</i></sub> |
| slobs_replay_buffer_state          | Replay Buffer status (as text)                                                                                             |
| slobs_replay_buffer                | Replay Buffer status (as true/false)<br><sub><i>True if Replay Buffer status is <b>NOT</b> </i>`OFFLINE`</sub>             |
| slobs_recording_status             | Recording status (as text)                                                                                                 |
| slobs_recording                    | Recording status (as true/false)<br><sub><i>True if Recording status is <b>NOT</b> </i>`OFFLINE`</sub>                     |
| slobs_streaming_status             | Streaming status (as text)                                                                                                 |
| slobs_streaming                    | Streaming status (as true/false)<br><sub><i>True if Streaming status is <b>NOT</b> </i>`OFFLINE`</sub>                     |
...
| Switch Scene                       | Switch scene within the current site collection<br><sub><i>Only works if scene is in the active site colleciton</i></sub> |
